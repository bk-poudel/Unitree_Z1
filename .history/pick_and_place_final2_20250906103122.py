...
waypoints = [
    initial_position,
    initial_position + np.array([0, 0, 0.2]),  # hover above bottle
...
orientations = [
    initial_orientation,  # 0-2000
    initial_orientation,  # 2000-4000
    rotated_orientation,  # 4000-6000
    rotated_orientation,  # 6000-8000
    rotated_orientation,  # 8000-10000
    rotated_orientation,  # 10000-12000
    rotated_orientation,  # 12000-14000
]
...
for step in range(len(trajectory)):
    ...
    if step == 8000:  # at bottle grasping point
        # Slowly close gripper from -1.5 to -0.3 over 200 timesteps
        for i in range(200):
            data.qpos[:6] = q_des[:6]  # keep arm steady
            # Linear interpolation for gripper position
            data.qpos[6] = -1.5 + (i / 199) * (0 + 1.5)
            mujoco.mj_step(model, data)
            viewer.sync()
            time.sleep(dt)
    mujoco.mj_step(model, data)
