...
target_position = np.array([0.5, 0.4, 0.18])  # Move 10cm forward
target_orientation = initial_orientation
trajectory = np.linspace(initial_position, target_position, num=200)
print_model_info(robot)
print(get_jacobian(robot, q))
# PD controller gains
# Cartesian impedance control parameters
K_cartesian = np.diag([10, 10, 10, 10, 10, 10])  # Stiffness
D_cartesian = np.diag([2, 2, 2, 2, 2, 2])  # Damping
steps = 0
while True:
