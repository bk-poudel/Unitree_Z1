...
for i in range(len(waypoints) - 1):
    ...
    interp_positions = [
        (1 - alpha) * p_start + alpha * p_end for alpha in np.linspace(0, 1, 5000)
    ]
    interp_orientations = interpolate_orientations(R_start, R_end, num_steps=5000)
...
for step in range(len(trajectory)):
    ...
    if step == 10000:  # at bottle grasping point
