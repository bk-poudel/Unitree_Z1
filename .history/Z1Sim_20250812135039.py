...
class Z1Sim:
    def __init__(self, interface_type="torque", render=True, dt=0.001, xml_path=None):
        ...
        # Check if assets directory exists, if not use script directory
        if os.path.exists(ASSETS_PATH):
            meshes_dir = ASSETS_PATH
        else:
            # Meshes might be in the same directory as the URDF
            meshes_dir = SCRIPT_DIR
            print(f"Warning: assets directory not found, using {meshes_dir} for meshes")
        try:
            self.pin_model = RobotWrapper.BuildFromURDF(urdf_path, meshes_dir)
        except Exception as e:
            print(f"Warning: Could not load Pinocchio model: {e}")
            print("Some kinematics functions may not work properly")
            self.pin_model = None
        # ROS 2 setup
        if not rclpy.ok():
            rclpy.init()
