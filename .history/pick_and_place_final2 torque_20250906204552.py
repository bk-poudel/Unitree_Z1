...
# ----------------------
kp_arm = np.array([1000, 2000, 1000, 1000, 1000, 1000])
kd_arm = np.array([200, 200, 150, 100, 50, 50])
kp_gripper = 500
kd_gripper = 50
...
for step in range(len(trajectory)):
    ...
    # Gripper target schedule
    if 6000 <= step < 6200:
        alpha = (step - 6000) / 200
        gripper_target = gripper_open + alpha * (gripper_closed - gripper_open)
    elif step >= 6200:
        gripper_target = gripper_closed
    # Gripper PD torque
    tau_gripper = kp_gripper * (gripper_target - q[6]) - kd_gripper * dq[6]
