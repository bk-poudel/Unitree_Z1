...
END_EFF_FRAME_ID = 7  # Frame ID for the end-effector in the Pinocchio model


def print_model_info(pin_model):
    ...
    for i, name in enumerate(pin_model.frames):
        print(f"ID: {i}, Name: {name.name}")


def get_gravity(pin_model, q):
    g = pin.rnea(pin_model, pin.Data(pin_model), q, np.zeros_like(q), np.zeros_like(q))
    return g[:6]


def get_jacobian(pin_model, q):
    J_temp = pin.computeFrameJacobian(pin_model, pin.Data(pin_model), q, END_EFF_FRAME_ID)
    J = np.zeros([6, 7])
    J[3:6, :] = J_temp[0:3, :6]
    J[0:3, :] = J_temp[3:6, :6]
    return J[:, :7]
