...
class Z1Sim:
    ...
    def solve_ik(
        ...
        for i in range(max_iter):
            ...
            task_error = np.concatenate([pos_error, orientation_weight * orientation_error])

            # Check if we've converged (for position only, can be modified)
            if np.linalg.norm(pos_error) < eps and np.linalg.norm(orientation_error) < eps:
                success = True
                break
            ...
            J_pinv = task_J.T @ np.linalg.inv(task_J @ task_J.T + lambda_dls * np.eye(6))
