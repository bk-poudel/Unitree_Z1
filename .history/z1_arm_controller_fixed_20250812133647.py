...
class Z1ArmController:
    ...
    def compute_impedance_control(
        ...
        # Compute pose error (simplified for when Pinocchio is not available)
            current_placement = SE3(current_pose[:3, :3], current_pose[:3, 3])
            target_placement = SE3(target_pose[:3, :3], target_pose[:3, 3])
...
def main():
    ...
    target_joints = np.array([0.1, 0.1, -0.5, 0.2, 0.1, 0.2])
    ...
    # Define target pose (move 10cm forward in Y)
