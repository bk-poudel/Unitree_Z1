...
def pd_cartesian_torque(
    ...
    """
    Compute joint torques to track Cartesian position and orientation targets.
    Args:
    ...
    tau_pos = J_pos.T @ (kp_pos * e_pos + kd_pos * v_error)
    # Orientation PD control (using quaternion difference)
    # The orientation error is a 3D vector.
    quat_error = np.zeros(3)
    # Using mju_quatSub as mju_quatDiff may not be available in all versions.
    # The order is (result, q1, q2) where result represents q1 - q2
    mujoco.mju_quatSub(quat_error, q_des, q_current)
    w_error = w_des - w_current
    tau_rot = J_rot.T @ (kp_rot * quat_error + kd_rot * w_error)
    # Total torque is the sum of position and orientation control torques
    tau = tau_pos + tau_rot
...
def main():
    ...
    q_des = data.body(ee_body_id).xquat.copy()
