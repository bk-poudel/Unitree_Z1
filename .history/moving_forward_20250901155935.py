...
def pd_cartesian_torque(
    ...
    # Compute the orientation error using rotation matrices.
    # The error vector is extracted from the skew-symmetric part of the relative rotation matrix.
    r_relative = r_des @ r_current.T
    quat_error = np.zeros(3)
    quat_error[0] = r_relative[2, 1] - r_relative[1, 2]
    quat_error[1] = r_relative[0, 2] - r_relative[2, 0]
    quat_error[2] = r_relative[1, 0] - r_relative[0, 1]
    # Use half of the error vector as a common approximation
    quat_error *= 0.5
