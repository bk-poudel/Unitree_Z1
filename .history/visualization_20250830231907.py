...
def main() -> None:
    ...
    site_id = model.body("gripperMover").id
    ...
    actuator_ids = np.array(
        ["motor1", "motor2", "motor3", "motor4", "motor5", "motor6", "motorGripper"]
    )
    # Initial joint configuration saved as a keyframe in the XML file.
    key_id = model.key("home").id
    # Mocap body we will control with our mouse.
    mocap_id = model.body("link06").mocapid[0]
    ...
    with mujoco.viewer.launch_passive(
        ...
        # Get initial position and set target position (10 cm in x direction)
        initial_pos = data.site(site_id).xpos.copy()
        target_pos = initial_pos.copy()
        target_pos[0] += 0.1  # Add 10 cm in x direction
        # Keep track of movement completion
        movement_complete = False
        while viewer.is_running():
            step_start = time.time()
            if not movement_complete:
                # Set the target position (10 cm in positive x direction)
                ...
                if np.linalg.norm(error_pos) < 0.005:  # 5mm tolerance
                    movement_complete = True
                    print("Movement complete: Moved 10 cm in positive X direction")
                # Orientation error.
