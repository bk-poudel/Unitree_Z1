...
class Z1ArmController:
    ...
    def __init__(self, 
        ...
        if mujoco_xml_path is None:
            mujoco_xml_path = str(Path(__file__).parent / "z1.xml")
        ...
        self.impedance_params = ImpedanceParams(
            kp=np.diag([1000, 1000, 1000, 100, 100, 100]),  # Position/orientation stiffness
    ...
    def get_end_effector_velocity(self) -> np.ndarray:
        ...
        if self.pin_model is not None:
            pin.computeFrameJacobian(self.pin_model, self.pin_data, self.q, self.ee_frame_id)
            J = self.pin_data.J
            return J @ self.dq
        else:
    ...
    def inverse_kinematics(self, target_pose: np.ndarray, 
                          ...
                          max_iter: int = 100,
                          tol: float = 1e-6) -> Tuple[np.ndarray, bool]:
        """
        ...
        if self.pin_model is None:
            print("Warning: Pinocchio not available, returning current joint configuration")
            return q_init, False
        ...
        for i in range(max_iter):
            ...
            pin.computeFrameJacobian(self.pin_model, self.pin_data, q_sol, self.ee_frame_id)
            J = self.pin_data.J
            
            # Damped least squares
            lambda_reg = 1e-6
            dq = np.linalg.solve(J.T @ J + lambda_reg * np.eye(self.n_joints), 
                               J.T @ error.vector)
    ...
    def compute_impedance_control(self, 
                                ...
                                external_wrench: Optional[np.ndarray] = None) -> np.ndarray:
        """
        ...
        desired_wrench = (self.impedance_params.kp @ pose_error + 
        ...
        desired_wrench[:3] = np.clip(desired_wrench[:3], 
    ...
    def set_impedance_params(self, kp: np.ndarray, kd: np.ndarray, ki: Optional[np.ndarray] = None):
    ...
    def step_simulation(self, control_torques: Optional[np.ndarray] = None):
        ...
        if control_torques is not None:
            ...
            for i, joint_id in enumerate(self.joint_ids):
                if i < len(control_torques) and joint_id >= 0 and joint_id < self.mj_model.nu:
    ...
    def run_joint_position_control(self, target_positions: np.ndarray, duration: float = 5.0, visualize: bool = True):
        """
