...
def inverse_kinematics(
    ...
    for _ in range(max_iters):
        ...
        dq_null = (np.eye(len(q_current)) - J_pinv @ J_full) @ 0.1 * (q_ref - q_current)
        dq = dq_task + dq_null
        q_current += dq
...
bottle_pos = np.array([0.2, 0.3, 0.1])
glass_pos = np.array([0.4, 0.0, 0.15])
waypoints = [
...
waypoints.append(glass_pos)  # same position
...
for i in range(len(waypoints) - 1):
    ...
    for alpha in np.linspace(0, 1, 100):
        trajectory.append((1 - alpha) * p_start + alpha * p_end)
        R_interp = R.from_matrix(R_start).interp(R.from_matrix(R_end), alpha)
        trajectory_orient.append(R_interp.as_matrix())
# ----------------------
# Main loop
# ----------------------
