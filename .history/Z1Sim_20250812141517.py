...
class Z1Sim:
    def __init__(self, interface_type="torque", render=True, dt=0.001, xml_path=None):
        ...
        # Use local z1_description directory for meshes
        z1_desc_dir = os.path.join(SCRIPT_DIR, "z1_description")
        meshes_dir = os.path.join(z1_desc_dir, "meshes")
        visual_meshes_dir = os.path.join(meshes_dir, "visual")
        collision_meshes_dir = os.path.join(meshes_dir, "collision")
        
        print(f"Looking for z1_description in: {z1_desc_dir}")
        print(f"Visual meshes: {visual_meshes_dir}")
        
        # Verify mesh directories exist
        if not os.path.exists(visual_meshes_dir):
            print(f"Warning: Visual meshes directory not found: {visual_meshes_dir}")
        if not os.path.exists(collision_meshes_dir):
            print(f"Warning: Collision meshes directory not found: {collision_meshes_dir}")
        
        try:
            # Method 1: Load with z1_description as base directory
            print("Attempting to load Pinocchio model with local z1_description...")
            self.pin_model = RobotWrapper.BuildFromURDF(urdf_path, SCRIPT_DIR)
            print("✓ Pinocchio model loaded successfully with local meshes!")
        except Exception as e1:
            print(f"✗ Local mesh loading failed: {e1}")
            try:
                # Method 2: Load without meshes (kinematics only)
                print("Loading without meshes (kinematics only)...")
                import pinocchio as pin
                model = pin.buildModelFromUrdf(urdf_path)
                self.pin_model = RobotWrapper(model)
                print("✓ Pinocchio model loaded without meshes!")
