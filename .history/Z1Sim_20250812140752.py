...
class Z1Sim:
    def __init__(self, interface_type="torque", render=True, dt=0.001, xml_path=None):
        ...
        # Use assets directory for meshes - check organized structure
        meshes_dir = ASSETS_PATH
        if not os.path.exists(meshes_dir):
            raise FileNotFoundError(f"Assets directory not found at {meshes_dir}")
        
        # Check for organized mesh structure
        visual_meshes_dir = os.path.join(meshes_dir, "meshes", "visual")
        collision_meshes_dir = os.path.join(meshes_dir, "meshes", "collision")
        
        # Debug: List available mesh files in different locations
        print(f"Looking for meshes in: {meshes_dir}")
        
        # Check root assets directory
        if os.path.exists(meshes_dir):
            root_mesh_files = [f for f in os.listdir(meshes_dir) if f.endswith(('.stl', '.dae'))]
            print(f"Found mesh files in root assets: {len(root_mesh_files)} files")
        
        # Check visual meshes directory
        if os.path.exists(visual_meshes_dir):
            visual_mesh_files = [f for f in os.listdir(visual_meshes_dir) if f.endswith(('.stl', '.dae'))]
            print(f"Found visual mesh files: {visual_mesh_files}")
        
        # Check collision meshes directory
        if os.path.exists(collision_meshes_dir):
            collision_mesh_files = [f for f in os.listdir(collision_meshes_dir) if f.endswith(('.stl', '.dae'))]
            print(f"Found collision mesh files: {collision_mesh_files}")
        
        # Try multiple mesh directories for Pinocchio
        mesh_search_paths = [
            meshes_dir,  # Root assets directory
            visual_meshes_dir,  # Visual meshes subdirectory
            collision_meshes_dir,  # Collision meshes subdirectory
        ]
        
        # Filter existing paths
        existing_mesh_paths = [path for path in mesh_search_paths if os.path.exists(path)]
        print(f"Mesh search paths: {existing_mesh_paths}")
        
        try:
            # Method 1: Try with primary mesh directory (visual preferred)
            if os.path.exists(visual_meshes_dir):
                print("Attempting to load Pinocchio model with visual meshes...")
                self.pin_model = RobotWrapper.BuildFromURDF(urdf_path, visual_meshes_dir)
                print("✓ Pinocchio model loaded successfully with visual meshes!")
            else:
                print("Attempting to load Pinocchio model with root assets directory...")
                self.pin_model = RobotWrapper.BuildFromURDF(urdf_path, meshes_dir)
                print("✓ Pinocchio model loaded successfully with root assets!")
        except Exception as e1:
            print(f"✗ Method 1 failed: {e1}")
            try:
                # Method 2: Try with multiple directories
                print("Attempting to load with multiple mesh directories...")
                self.pin_model = RobotWrapper.BuildFromURDF(urdf_path, existing_mesh_paths)
                print("✓ Pinocchio model loaded successfully with multiple directories!")
            except Exception as e2:
                print(f"✗ Method 2 failed: {e2}")
                try:
                    # Method 3: Try with collision meshes
                    if os.path.exists(collision_meshes_dir):
                        print("Attempting to load with collision meshes...")
                        self.pin_model = RobotWrapper.BuildFromURDF(urdf_path, collision_meshes_dir)
                        print("✓ Pinocchio model loaded successfully with collision meshes!")
                    else:
                        raise Exception("No collision mesh directory found")
                except Exception as e3:
                    print(f"✗ Method 3 failed: {e3}")
                    try:
                        # Method 4: Load without meshes (kinematics only)
                        ...
                        print("⚠️  Could not load Pinocchio model - kinematics functions will not work")
                        self.pin_model = None
        # ROS 2 setup
        if not rclpy.ok():
            rclpy.init()
