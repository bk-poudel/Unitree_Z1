...
class Z1Sim:
    ...
    def send_constant_torque(self, torque_command):
        ...
        pin.computeGeneralizedGravity(self.pin_model, self.pin_data, q[:6])
...
# Example usage with constant torque:
mjcf_path = "scene.xml"
urdf_path = "z1.urdf"
sim = Z1Sim(mjcf_path, urdf_path)
sim.reset()
viewer = mujoco.viewer.launch_passive(sim.model, sim.data)
# Constant desired torque (modify as needed)
desired_torque = np.array([0.5, 0, 0, 0, 0, 0])
start_time = time.time()
while time.time() - start_time < 10:
    sim.send_constant_torque(desired_torque)
    viewer.sync()
