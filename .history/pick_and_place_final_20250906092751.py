...
# ----------------------
# Build complete trajectory including home pose movement
# ----------------------
# Initialize CSV file
with open("joint_states.csv", "w", newline="") as csvfile:
    writer = csv.writer(csvfile)
# First, interpolate from initial_position to home_pos over 2 seconds
...
for i in range(num_home_steps):
    ...
    home_trajectory.append(interp_q[:6])  # Only arm joints for trajectory
# Get end-effector pose at home position for task-space planning
data.qpos[:7] = home_pos
mujoco.mj_forward(model, data)
q_home, _ = get_states(data)
home_pose = get_ee_pose(q_home, robot)
initial_ee_position = home_pose[:3, 3]
initial_ee_orientation = home_pose[:3, :3]
...
pour_orientation = (
    initial_ee_orientation @ R.from_euler("x", -90, degrees=True).as_matrix()
)
waypoints.append(glass_pos)
orientations.append(pour_orientation)
# Return upright
waypoints.append(glass_pos + np.array([0, 0, 0.2]))
orientations.append(initial_ee_orientation)
...
for step in range(total_steps):
    ...
    if step < len(home_trajectory):
        # Phase 1: Move to home position (joint space interpolation)
        data.qpos[:7] = home_trajectory[step]  # All 7 joints including gripper
    else:
