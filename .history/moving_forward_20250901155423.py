...
def pd_cartesian_torque(
    ...
    # Manually compute the quaternion error using a robust method
    q_inv_current = np.zeros(4)
    mujoco.mju_negQuat(q_inv_current, q_current)
    q_error_quat = np.zeros(4)
    mujoco.mju_mulQuat(q_error_quat, q_des, q_inv_current)
    quat_error = (
        2.0 * q_error_quat[:3]
    )  # Standard approximation for the rotational error vector
