...
class Z1sim:
    def __init__(self, xml_path="scene.xml", urdf_path="z1.urdf"):
...
if __name__ == "__main__":
    sim = Z1sim("scene.xml", "z1.urdf")
    sim.launch_viewer()
    torque = np.array([0.0, 0.0])
    step_size = np.array([0.1, 0.1])  # Change step size as needed
    while True:
        sim.apply_joint_torque(["joint1", "joint2"], torque)
        sim.step()
        ...
        torque += step_size  # Increase torque
