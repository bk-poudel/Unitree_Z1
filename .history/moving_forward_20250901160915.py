...
def pose_error_log(T_desired, T_current):
    """
    Compute 6D logarithmic pose error in the base frame.
    Args:
        T_desired (np.ndarray): Desired homogeneous transformation matrix (4x4).
        T_current (np.ndarray): Current homogeneous transformation matrix (4x4).
    Returns:
        np.ndarray: 6D pose error vector [dx, dy, dz, d_rot_x, d_rot_y, d_rot_z].
    """
...
def pd_cartesian_torque(
    ...
    Args:
        ...
        r_des (np.ndarray): Desired end-effector orientation (3x3 rotation matrix).
    ...
    try:
        ...
        print("Error: 'gripperMover' body not found in the model.")
        return np.zeros(model.nu)
    # Current EE position, orientation, and velocities
    x_current = data.body(ee_body.id).xpos
    r_current = data.body(ee_body.id).xmat.reshape(3, 3)
    ...
    # Current EE spatial velocity
    v_current = J_pos @ data.qvel
    ...
    T_current[:3, 3] = x_current
    # Compute 6D spatial error vector using the new function
    spatial_error = pose_error_log(T_desired, T_current)
    # Compute 6D spatial velocity error vector
    spatial_vel_error = np.hstack((x_dot_des - v_current, w_des - w_current))
...
# ----------------------------
def create_trajectory(p0, p1, total_time=5.0, n_points=2000):
    """
...
def main():
    """
    Main function to run the MuJoCo simulation with a Cartesian PD controller.
    """
    # Load model from the provided XML file
    try:
    ...
    r_des = data.body(ee_body_id).xmat.reshape(3, 3).copy()
    w_des = np.zeros(3)  # No desired angular velocity
    ...
    try:
        while viewer_handle.is_running():
            ...
            current_x_dot_des = (
                x_dot_des if clamped_sim_time < total_time else np.zeros(3)
            )
            # Compute torque
            ...
            if int(sim_time / dt) % 200 == 0:
                ...
                r_current = data.body(ee_body_id).xmat.reshape(3, 3)
                # Construct transformation matrices for logging
                T_desired = np.eye(4)
                T_desired[:3, :3] = r_des
                T_desired[:3, 3] = x_des
                T_current = np.eye(4)
                ...
                print(f"Transformation Matrix Error (6D): {spatial_error}")
