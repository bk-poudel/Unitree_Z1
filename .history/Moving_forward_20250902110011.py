...
home_pos = np.array([0, 0.785, -0.261, -0.523, 0, 0, 0.785])
...
while True:
    ...
    if index < 100 or (index > 200 and index < 300):
        send_torques(model, data, torque, viewer, -1.5)
    elif index >= 100 and index <= 200:
        torque = np.array([0, 0, 0, 0, 0, 3])
        send_torques(model, data, torque, viewer, 0)
    else:
        torque = np.array([0, 0, 0, 0, 0, -3])
        send_torques(model, data, torque, viewer, 0)
    index = index + 1
