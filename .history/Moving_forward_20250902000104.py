...
target_position = initial_position + np.array([0, -0.3, 0.25])  # Move 10cm forward
target_orientation = initial_orientation
trajectory = np.linspace(initial_position, target_position, num=10000)
print_model_info(robot)
print(get_jacobian(robot, q))
# PD controller gains
# Cartesian impedance control parameters
K_cartesian = np.diag([200, 200, 200, 200, 200, 200])  # Stiffness
D_cartesian = np.diag([10, 10, 10, 10, 10, 10])  # Damping
for target in trajectory:
    ...
    torque = J.T @ force + get_gravity(robot, q)
