...
def print_model_info(pin_model=pin_model):
    """Prints the names and IDs of all bodies and frames in the model."""
    print("--- Bodies ---")
    for i, name in enumerate(pin_model.body_names):
        print(f"ID: {i}, Name: {name.decode('utf-8')}")
    print("\n--- Frames ---")
    for i, name in enumerate(pin_model.frame_names):
        print(f"ID: {i}, Name: {name.decode('utf-8')}")


def get_gravity(q):
    g = pin_model.gravity(q)
    return g[:6]
