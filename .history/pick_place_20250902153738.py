...
K_cartesian = np.diag([1500, 1500, 1500, 1500, 1500, 1500])  # Stiffness matrix
D_cartesian = np.diag([100, 100, 100, 100, 100, 100])  # Damping matrix
segment_index = 0
...
while viewer.is_running():
    ...
    full_torque = np.concatenate([torque, [gripper_command]])
    # Apply the torques and gripper command to the robot
    send_torques(model, data, full_torque, viewer)
