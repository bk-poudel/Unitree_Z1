...
class Z1ArmController:
    ...
    def _init_mujoco(self):
        ...
        try:
            ...
            for i in range(self.n_joints):
                ...
                if joint_name in [self.mj_model.joint(j).name for j in range(self.mj_model.njnt)]:
                    self.joint_names.append(joint_name)
                    self.joint_ids.append(mujoco.mj_name2id(self.mj_model, mujoco.mjtObj.mjOBJ_JOINT, joint_name))
    ...
    def set_impedance_params(self, kp: np.ndarray, kd: np.ndarray, ki: np.ndarray = None):
