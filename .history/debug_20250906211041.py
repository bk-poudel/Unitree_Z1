...
def smooth_gripper_transition(
    ...
    for i in range(steps):
        ...
        tau_gripper = kp_gripper * (gripper_pos - q[6]) + kd_gripper * (0 - dq[6])
...
while step < len(trajectory):
    ...
    if step == 2000 and not gripper_transition_active:
        ...
        # Normal operation - maintain current gripper position with torque control
        if step < 2000:
            gripper_target = 0.0
        elif step < 6000:
            gripper_target = -1.5
            ...
            gripper_target = -0.8
        tau_gripper = kp_gripper * (gripper_target - q[6]) + kd_gripper * (0 - dq[6])
        data.qfrc_applied[6] = tau_gripper
