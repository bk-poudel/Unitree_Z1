...
class Z1Sim:
    def __init__(self, interface_type="torque", render=True, dt=0.001, xml_path=None):
        ...
        if os.path.exists(meshes_dir):
            root_mesh_files = [
                f for f in os.listdir(meshes_dir) if f.endswith((".stl", ".dae"))
            ]
        ...
        if os.path.exists(visual_meshes_dir):
            visual_mesh_files = [
                f for f in os.listdir(visual_meshes_dir) if f.endswith((".stl", ".dae"))
            ]
        ...
        if os.path.exists(collision_meshes_dir):
            collision_mesh_files = [
                f
                for f in os.listdir(collision_meshes_dir)
                if f.endswith((".stl", ".dae"))
            ]
        ...
        existing_mesh_paths = [
            path for path in mesh_search_paths if os.path.exists(path)
        ]
        ...
        try:
            ...
            if os.path.exists(visual_meshes_dir):
                ...
                self.pin_model = RobotWrapper.BuildFromURDF(
                    urdf_path, visual_meshes_dir
                )
                print("✓ Pinocchio model loaded successfully with visual meshes!")
            else:
                print(
                    "Attempting to load Pinocchio model with root assets directory..."
                )
            ...
            try:
                ...
                self.pin_model = RobotWrapper.BuildFromURDF(
                    urdf_path, existing_mesh_paths
                )
                print(
                    "✓ Pinocchio model loaded successfully with multiple directories!"
                )
                ...
                try:
                    ...
                    if os.path.exists(collision_meshes_dir):
                        ...
                        self.pin_model = RobotWrapper.BuildFromURDF(
                            urdf_path, collision_meshes_dir
                        )
                        print(
                            "✓ Pinocchio model loaded successfully with collision meshes!"
                        )
                    else:
                    ...
                    try:
                        ...
                        print(
                            "✓ Pinocchio model loaded without meshes (kinematics only)!"
                        )
