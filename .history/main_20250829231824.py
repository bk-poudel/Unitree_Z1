...
def inverse_kinematics_with_orientation(model, data, body_id, target_pos, target_quat, max_iter=100, tol=1e-3):
...
def main():
    ...
    # Move EE in x direction only, 10 cm, keep orientation unchanged
    start_pos = [0.0, 0.0, 0.2]
    end_pos = [0.5, 0.0, 0.2]  # 10 cm in x
    steps = 4000
    x_path = np.linspace(start_pos[0], end_pos[0], steps)
    ee_body_id = get_end_effector_body_id(model)
    with mujoco.viewer.launch_passive(model, data) as viewer:
        print(
            "Robot end effector moving 10 cm in x direction. Orientation is kept unchanged. Press ESC to exit."
        )
        for x in x_path:
            target = np.array([x, start_pos[1], start_pos[2]])
            # Only position is targeted, orientation remains unchanged
            inverse_kinematics(model, data, ee_body_id, target)
            mujoco.mj_step(model, data)
            viewer.sync()

    # Move EE in x direction only, 10 cm, keeping the initial orientation
    start_pos = [0.5, 0.0, 0.2]
    end_pos = [1.0, 0.0, 0.2]  # 10 cm in x
    x_path = np.linspace(start_pos[0], end_pos[0], steps)
    mujoco.mj_forward(model, data)
    initial_quat = get_end_effector_quat(data, ee_body_id)
    with mujoco.viewer.launch_passive(model, data) as viewer:
        print("Robot end effector moving 10 cm in x direction, orientation preserved. Press ESC to exit.")
        for x in x_path:
            target = np.array([x, start_pos[1], start_pos[2]])
            inverse_kinematics_with_orientation(model, data, ee_body_id, target, initial_quat)
            mujoco.mj_step(model, data)
            viewer.sync()
